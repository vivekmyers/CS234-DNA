...
def build_mlp(
  ...
  with tf.variable_scope(scope):
    for i in range(n_layers):
      layers.append(tf.layers.dense(inputs=layers[-1], units=size, activation=tf.nn.relu))
    result = tf.layers.dense(inputs=layers[-1], units=output_size, activation=output_activation)
  return result  
  #######################################################
  #########          END YOUR CODE.          ############
...
class PG(object):
  ...
  def build_policy_network_op(self, scope = "policy_network"):
    ...
    out = build_mlp(self.observation_placeholder, self.action_dim, scope, self.config.n_layers, self.config.layer_size, None)
    with tf.variable_scope(scope):    
        if self.discrete:
          action_logits = out
          self.sampled_action = tf.squeeze(tf.multinomial(out, 1), axis=1)
          ...
          self.sampled_action = tf.random_normal([self.action_dim]) * tf.exp(log_std) + action_means  
          self.logprob = tf.contrib.distributions.MultivariateNormalDiag(action_means, tf.exp(log_std)).prob(self.action_placeholder)
    #######################################################
    #########          END YOUR CODE.          ############
  ...
  def add_baseline_op(self, scope = "baseline"):
    ...
    self.baseline = tf.squeeze(build_mlp(self.observation_placeholder, 
                            1, scope, self.config.n_layers, self.config.layer_size, None))
    self.baseline_target_placeholder = tf.placeholder(tf.float32, [None])
    self.update_baseline_op = tf.train.AdamOptimizer(self.lr).minimize(tf.losses.mean_squared_error(self.baseline, self.baseline_target_placeholder))
    #######################################################
    #########          END YOUR CODE.          ############
  ...
  def get_returns(self, paths):
    ...
    for path in paths:
      ...
      #######################################################
      #########   YOUR CODE HERE - 5-10 lines.   ############
      returns = [sum((self.config.gamma ** (i - j)) * rewards[i] 
  ...
  def update_baseline(self, returns, observations):
    ...
    self.sess.run(self.update_baseline_op, feed_dict={
      self.baseline_target_placeholder: self.calculate_advantage(returns, observations),
